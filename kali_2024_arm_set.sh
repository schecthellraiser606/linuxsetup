...
sudo apt install dnsmasq urlcrazy dirsearch libreoffice bloodhound ghidra gobuster golang php-curl terminator seclists sshuttle peass jd-gui keepassx qemu-user powershell-empire snmp-mibs-downloader netexec cadaver hydra medusa irssi ltrace sippts burpsuite bloodhound-ce-python -y
sudo apt install mariadb-client-compat libfreetype6-dev autorecon bloodyad faketime krb5-pkinit openjdk-11-jdk -y
...
sudo git clone https://github.com/pinksawtooth/ghidra_scripts pink_scripts
...
sudo git clone https://github.com/AllsafeCyberSecurity/Ghidra_FIDB
sudo wget https://github.com/LAC-Japan/Ghidra_AntiDebugSeeker/releases/download/v1.2/Ghidra-AntiDebugSeeker.zip
